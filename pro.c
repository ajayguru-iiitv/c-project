...
/*function to store the data of a visitor in customer.dat*/
void signup(){
	FILE *p;
...
/*function to add a project in a database file which is project.dat*/
void add_project(char n[]){
	struct pro e;
	FILE *p;
	p = fopen("project.dat","a+");
	printf("enter the name of the customer\n");
	scanf("%s",e.name);
	printf("enter sevent name\n");
	scanf("%s",e.event);
...
void view_profile(char n[]){
	...
	while(fread(&s,sizeof(s),1,p)==1)
    {
        ...
        {
        printf("\t\t%s\n%s\n%s\n%s\n%s\n%s\n%s\n",s.name,s.id,s.insal,s.experience,s.accnum,s.password,s.project);
        //printf("IF KE ANDAR HU\n");
	skipper=0;
        break;
        }
    //printf("YO IN THE LOOP\n");
    }
	if(skipper==1)
...
void admin_customer(){
	...
	remove("employee.dat");
	rename("COPY.dat","customer.dat");
	admin();
	}
void admin_partner(){
	printf("1. Add partner\n2. remove partner");
	int select;
	scanf("%d",&select);
	switch(select){
		case 1:
...
void delete_partner(char n[]){
	...
	remove("partner.dat");
	rename("COPY.dat","partner.dat");
	admin();
}
void admin_project(char n[]){
	printf("1. view project\n2. edit team members\n3. update project\n4. delete project");
	int select;
	switch(select){
		case 1:
		view_project(n);
